...
{
	twl4030_i2c_write_u8(TWL4030_MODULE_GPIO, 0x80, REG_GPIODATADIR1);
...
static struct regulator_consumer_supply devkit8000_vdds_dsi_supply = {
	.supply = "vdds_dsi",
	.dev	= &devkit8000_dss_device.dev,
};
...
static struct regulator_init_data devkit8000_vpll2 = {
	...
	.num_consumer_supplies	= 1,
	.consumer_supplies	= &devkit8000_vdds_dsi_supply,
};

static const struct twl4030_resconfig devkit8000_resconfig[] = {
...
static struct twl4030_power_data devkit8000_power_data = {
	.resource_config	= devkit8000_resconfig,
...
{
	int i;
	for (i = 0; i < ARRAY_SIZE(devkit8000_pins); ++i) {
		omap_ctrl_writew(devkit8000_pins[i].mux_val,
				 devkit8000_pins[i].mux_reg);
	}
	return 0;
}

#else
static inline int devkit8000_mux_init() { return 0; }
#endif /* CONFIG_DEVKIT8000_MUX */
...
static void __init devkit8000_flash_init(void)
{
	...
	if (nandcs < GPMC_CS_NUM) {
		...
		devkit8000_nand_data.gpmc_baseaddr = (void *) (gpmc_base_add);
